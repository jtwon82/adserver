...
public class AdbnMobile1 extends HttpServlet {
	private DumpObject dumpObj;
	private ApplicationContext appContext;
	public ManagementCookie manageCookie;
	static Logger logger = Logger.getLogger(AdbnMobile1.class);
		...
		try {
			...
			if( (adgubun==null || adgubun.equals("")) && ms.getAccept_ad().equals("Y") ){
				try{
					...
					String baseadKey=us+"_" + (imgtype.equals("1")?"i250_250":ms.getAD_TYPE());
			...
			if(adgubun==null || adgubun.equals("")){
				try{
					...
					if(h_banner==null || h_banner.equals("")){
						...
						logger.debug("TEST h_banner= "+h_banner);
						logger.debug("TEST htmlStr.toString()= "+htmlStr.toString());
						return;
					}
				}catch(Exception e){
			...
			String r_site_name  = scfg.getSite_name();
			...
			try{
				...
				cntsr_tmp= Integer.parseInt(cntsr);
			}catch(Exception e){
				cntad_tmp=1;
				cntsr_tmp=1;
			}
			...
			if( adgubun.equals("SR") || adgubun.equals("SP") || adgubun.equals("ST") ){
				...
				html_flash.append("{\"t0\":\"\",\"pcode\":\""+scfg.getPcode()+"\",\"site_name\":\""+r_site_name
				...
				html_flash.append("{\"t0\":\"\",\"desc\":\""+ scfg.getSite_descm()
						+"\",\"img\":\""+ img +"\",\"site_name\":\""+ r_site_name+"\",\"purl\":\""+ site_url 
						+"\",\"logo\":\""+ scfg.getImgname_logo() +"\"}");
				...
				if( list!=null && list.size()>0 ){
					...
					while( it.hasNext() && max>0 ){
						...
						html_flash.append(",{\"t"+(max+1)+"\":\"\",\"desc\":\""+ex_item.getSite_descm()
								+"\",\"img\":\""+ img +"\",\"site_name\":\""+ex_item.getSite_name()+"\",\"purl\":\""+ site_url 
								+"\",\"logo\":\""+ ex_item.getImgname_logo() +"\"}");
				...
				html_flash.append("{\"t0\":\"\",\"desc\":\""+ scfg.getSite_descm()
						+"\",\"img\":\""+ img +"\",\"site_name\":\""+ r_site_name
						+"\",\"purl\":\""+ site_url +"\",\"logo\":\""+ scfg.getImgname_logo() +"\"}");
				...
				if( list!=null && list.size()>0 ){
					...
					while( it.hasNext() && max>0 ){
						...
						html_flash.append(",{\"t"+(max+1)+"\":\"\",\"desc\":\""+ex_item.getSite_descm()+"\",\"img\":\""+ img 
								+"\",\"site_name\":\""+ ex_item.getSite_name() +"\",\"purl\":\""+ site_url 
								+"\",\"logo\":\""+ ex_item.getImgname_logo() +"\"}");
