...
public class IAdbn extends HttpServlet {
	private DumpObject dumpObj;
	private ApplicationContext appContext;
	public ManagementCookie manageCookie;
	static Logger logger = Logger.getLogger(IAdbn.class);
	public void doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			*/
			...
			String delay=request.getParameter("delay")==null ? "" : request.getParameter("delay");
						...
						try{
							...
							if(shhm==null || shhm.size()==0){
							    ...
							    for(String me:it){
							    	...
							    	logger.debug("iadbn[293] shop_log1 "+shop_log1 );
							    ...
							    logger.debug("iadbn[347] scfg "+scfg);
			...
			if( adgubun.length()==0 && ms.getAccept_ad().equals("Y") ){	// 베이스 광고 ..
				...
				Document doc = (Document) RFServlet.instance.xml.get("iadbn_ebay");
//				try{
//					doc = Jsoup.parse(new File("/home/dreamsearch/public_html/adnet/iadbn_ebay.xml"), "UTF-8", "http://"+domain+"/");
//				}catch(Exception e){
//					try{
//						doc = Jsoup.parse(new File("D:/work/enliple/www/webapp/public_html/adnet/iadbn_ebay.xml"), "UTF-8", "http://"+domain+"/");
//					}catch(Exception e1){}
//				}

				Element site= doc.select("ebay site ad").get(0);
				String cho_html = site.select("tags").text();
				String cho_sitecode = "";
				String cho_limit_media = site.select("limit_media").text();
				String dispo = manageCookie.getCookieInfo(request,"dispo");
				
				if( cho_limit_media.equals("") ){
					cho_sitecode=site.select("sitecode").text();
					
				}else if( !cho_limit_media.equals("") && s.equals(cho_limit_media) ){
					cho_sitecode=site.select("sitecode").text();
				...
				if( psb.equals("99") || new Random().nextInt(2)==0 || cho_sitecode.equals("") ){
					logger.debug("베이스 광고 ..");
					...
					cho_html = cho_html.replace("{s}", s);
					cho_html = cho_html.replace("{u}", site.select("userid").text());
					cho_html = cho_html.replace("{site_code}", cho_sitecode );
			...
			String rurl="";
			String slink="servlet/idrc?kno="+r_kno+"&no="+r_no+"&s="+s+"&gb="+adgubun+"&sc="+r_site_code+"&mc="+s+"&mcgb="+mcgb+"&u="+r_userid+"&pcode="+r_pcode+"&clk_param="+URLEncoder.encode(clk_param,"euc-kr");
			...
			if( scfg.isAdnew() ){
				out.print(scfg.getAdnew_html());
			}else{
				Date edate = new Date( );
//				String sss= FileUtil.readFile("/home/dreamsearch/public_html/ad/iadbn_base.html");
//				if( sss.equals("") ){
//					sss= FileUtil.readFile("D:/work/enliple/www/webapp/public_html/ad/iadbn_base.html");
//				}
				String sss=(String) RFServlet.instance.xml.get("iadbn_base");
				if(sss.equals(""))logger.error("adbnfloating iadbn_basenone ");
				
				try{
					if( mcgb==null )mcgb="";
					if( r_no==null )r_no="0";
					if( r_kno==null )r_kno="";
					if( scfg.getPcode()==null ) scfg.setPcode("");
					
					sss= sss.replace("{{site_code}}", scfg.getSite_code());
					sss= sss.replace("{{adgubun}}", adgubun);
					sss= sss.replace("{{gb}}", gb);
					sss= sss.replace("{{userid}}", scfg.getUserid());
					sss= sss.replace("{{no}}", r_no);
					sss= sss.replace("{{us}}", us);
					sss= sss.replace("{{s}}", s);
					sss= sss.replace("{{pcode}}", scfg.getPcode());
					sss= sss.replace("{{mcgb}}", mcgb);
					sss= sss.replace("{{kno}}", r_kno);
					sss= sss.replace("{{openURL}}", "http://"+domain+"/"+slink);
					sss= sss.replace("{{shopurl}}", URLEncoder.encode(r_site_url,"UTF-8"));
					sss= sss.replace("{{diff}}", (edate.getTime() - sdate.getTime())+"" );
				}catch(Exception ex){
					logger.error("iadbn[615] err "+ ex);
				}
				out.print(sss);
			}
			...
			logger.error("IAdbn:"+e+",debug="+debug); 
		}
	}
	...
	public void init() throws ServletException {
		logger.info("IAdbnServlet started");
		if (appContext == null) {
